...
best <-  glmer(maxn_sum ~ bait + (1|location/site), data = sum.stage,
              family = "poisson")
...
maxnSCsum.w.post <-
  ...
  annotate("text", x = 1.5, y = 2.75, label = "p = 0.012", size = 3) +
  ...
  annotate("text", x = 2, y = 2.85, label = "p = 0.260", size = 3)+
  ## Octopus vs Pilchard
  ...
  annotate("text", x = 2.5, y = 2.95, label = "p = 0.341", size = 3)
...
#######################################################
date <- glmer(maxn_sum ~ bait +  date + (1|location/site), data = sum.stage,
      family = "poisson")

summary(date)

# date_nest <- glmer(maxn_sum ~ bait + depth_m + (1|date) + (1|location/site), data = sum.stage,
#               family = "poisson")
# 
# summary(date_nest)

tod <- glmer(maxn_sum ~ bait + time_hr + (1|location/site), data = sum.stage,
             family = "poisson")
summary(tod)
AIC(best, tod)

tod.nobait <- glmer(maxn_sum ~  time_hr + (1|location/site), data = sum.stage,
             family = "poisson")
summary(tod.nobait)
AIC(best, tod.nobait)

tod_fact <- glmer(maxn_sum ~ bait + time_block + (1|location/site), data = sum.stage,
             family = "poisson")
summary(tod_fact)
AIC(best, tod_fact)
